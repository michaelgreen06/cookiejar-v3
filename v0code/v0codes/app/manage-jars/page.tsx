...
import { Search, Wallet, ArrowLeft, ExternalLink, Settings } from "lucide-react"

import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Card, CardContent, CardFooter } from "@/components/ui/card"
import { type CookieJar, getJarsOwnedByAddress, testAddresses } from "@/mock/jar-data"

export default function ManageJarsPage() {
  ...
  const handleSearch = () => {
    ...
    setTimeout(() => {
      ...
      // Get jars owned by the entered address using our mock data
      if (address.trim().length > 0) {
        const ownedJars = getJarsOwnedByAddress(address)
        setJars(ownedJars)
      } else {
        setJars([])
      }
  ...
  return (
    <div className="container max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">Manage My Cookie Jars</h1>
      </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <Input
                placeholder="Enter jar owner address (0x...)"
                value={address}
      ...
      {isLoading && (
        <div className="text-center py-12">
          ...
          <p className="mt-2 text-muted-foreground">Searching for your cookie jars...</p>
        </div>
      )}
      ...
      {searchPerformed && !isLoading && (
        <div className="space-y-6">
          {jars.length > 0 ? (
            <>
              <h2 className="text-lg font-medium">Found {jars.length} cookie jars you own</h2>
                ...
                {jars.map((jar) => (
                  <Card key={jar.id} className="overflow-hidden">
                    ...
                    <CardFooter className="bg-muted/50 px-6 py-3">
                      <Link href={`/manage-jar/${jar.id}`} className="w-full">
            ...
            <div className="text-center py-12 bg-muted/20 rounded-lg border">
              <div className="max-w-md mx-auto space-y-6">
                <p className="text-lg">Looks like the provided address doesn't own any cookie jars 😢</p>
